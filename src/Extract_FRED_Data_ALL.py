...
def process_releases():
    ...
    WriteAzureBlob.writeDataframeToBlob("allraw", "FRED_releases/FRED_releases.parquet", releases_df)
    global pagesRead
    pagesRead += 1
...
def process_series_for_release(release_id):
    ...
    WriteAzureBlob.writeDataframeToBlob("allraw", f"FRED_series/FRED_series_{release_id}.parquet""", series_df)
    global pagesRead
    pagesRead += 1
...
def process_observations_for_series(series_id):
    ...
    WriteAzureBlob.writeDataframeToBlob("allraw", f"FRED_observations/FRED_observations_{series_id}.parquet""", observation_df)
    global pagesRead
    pagesRead += 1
