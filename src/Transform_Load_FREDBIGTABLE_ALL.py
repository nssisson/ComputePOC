...
def main():
    ...
    releases_df = asyncio.run(QueryAzureBlobParquet.query("allraw", "FRED_releases"))
    releases_time = time.time() - step_start_time

    step_start_time = time.time()
    series_df = asyncio.run(QueryAzureBlobParquet.query("allraw", "FRED_series"))
    series_df["releases_id"] = series_df['source_file'].str.extract(r'_(\d+)\.parquet').astype(int)
    series_time = time.time() - step_start_time

    step_start_time = time.time()
    observation_df = asyncio.run(QueryAzureBlobParquet.query("allraw", "FRED_observations"))
    ...
    WriteAzureBlob.writeDataframeToBlob("staging", f"{timestamp}/FRED_BigTable/FRED_BigTable_ALL.parquet", merged)
