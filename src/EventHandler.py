...
loggingContainerName = os.environ.get('LOGGING_CONTAINER_NAME')
print(queueName)
...
try:
    ...
    response = task()
    WriteAzureBlob.writeJsonToBlob(loggingContainerName, f"{request['ExecutionId']}/{request['Process']}.json", response)
